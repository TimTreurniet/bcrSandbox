        ...
        <b-row v-for="bridges in groupedBridges" v-bind:key="bridges.id">
          <b-col v-for="bridge in bridges" v-bind:key="bridge.id">
            <b-card
              class="bridge-card"
              :title="bridge.name"
              :img-src="getImgUrl(bridge.filename)"
              img-width="80px"
              img-height="50%"
              img-top
            >
              ...
              >Laatste update: {{frontEndTimeFormat(bridge.updated_at)}}</b-card-text>
              <b-button
                variant="primary"
                :to="{ name: 'bridge-selected', params: { name: bridgeSelected }}"
              >Start bediening "{{bridge.name}}"</b-button>
            </b-card>
          </b-col>
        </b-row>
      </div>
